...
export async function POST(request: NextRequest) {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      customer: customerId,
      mode: 'payment',
      // Enable ALL Stripe payment methods for maximum flexibility
      payment_method_types: [
        'card',           // Credit/Debit cards
        'us_bank_account', // ACH bank transfer
        'affirm',         // Affirm BNPL (3-36 months)
        'klarna',         // Klarna BNPL (4 payments or financing)
        'afterpay_clearpay', // Afterpay (4 payments)
