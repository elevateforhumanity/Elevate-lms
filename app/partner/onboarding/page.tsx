...
import { 
  ...
  FileText, Users, Circle, ArrowRight, 
          ...
          {[1, 2, 3, 4, 5].map((s) => (
            <div key={s} className="flex items-center">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold ${
                ...
                {s < step ? <Circle className="w-5 h-5" /> : s}
              </div>
          ...
          <div className="flex justify-between mt-8 pt-6 border-t border-slate-200">
            ...
            {step < 5 ? (
              ...
              <button onClick={handleSubmit} disabled={isSubmitting || !formData.agreedToTerms}
                ...
                {isSubmitting ? <><Loader2 className="w-5 h-5 animate-spin" /> Submitting...</> : <>Submit Application <Circle className="w-5 h-5" /></>}
              </button>
            )}
          </div>
        </div>
