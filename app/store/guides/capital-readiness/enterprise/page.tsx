...
import { ArrowLeft, Building2, Shield, Globe, Users, FileCheck, Scale, Briefcase, Circle2, AlertTriangle, Clock, DollarSign } from "lucide-react";
                ...
                <ul className="space-y-3">
                  {[
                    ...
                    <li key={i} className="flex items-start gap-3 text-gray-300">
                      <Circle2 className="w-5 h-5 text-green-400 flex-shrink-0 mt-0.5" />
                      {item}
                    </li>
                  ))}
                </ul>
                    ...
                    <ul className="grid md:grid-cols-2 gap-2">
                      {phase.tasks.map((task, j) => (
                        <li key={j} className="flex items-center gap-2 text-gray-300 text-sm">
                          <Circle2 className="w-4 h-4 text-green-400 flex-shrink-0" />
                          {task}
                        </li>
                      ))}
                    </ul>
                ...
                <ul className="space-y-2 mb-4">
                  {stakeholder.responsibilities.map((resp, j) => (
                    <li key={j} className="flex items-start gap-2 text-gray-300 text-sm">
                      <Circle2 className="w-4 h-4 text-cyan-400 flex-shrink-0 mt-0.5" />
                      {resp}
                    </li>
                  ))}
                </ul>
              ...
              <div key={i} className="bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 p-6">
                ...
                <ul className="space-y-2">
                  {pricing.includes.map((item, j) => (
                    <li key={j} className="flex items-center gap-2 text-gray-300 text-sm">
                      <Circle2 className="w-4 h-4 text-emerald-400" />
                      {item}
                    </li>
                  ))}
                </ul>
